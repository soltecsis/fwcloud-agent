...
lazy_static! {
  ...
  pub static ref PLUGINS_NAMES: Regex = Regex::new("^(test|openvpn|openvpn-2fa|wireguard|ipsec|geoip|ntopng|suricata|keepalived|zeek|elasticsearch|filebeat|websafety|dnssafety|kibana|logstash|isc-bind9|isc-dhcp|haproxy|irqbalance)$").unwrap();
