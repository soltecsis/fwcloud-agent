...
impl Config {
  pub fn new() -> Result<Self> {
    ...
    let mut cfg = Config {
      ...
      bind_port: env::var("BIND_PORT").unwrap_or_else(|_| String::from("33033")).parse::<u16>().unwrap_or(33033),
      workers: env::var("WORKERS").unwrap_or_else(|_| cpus.to_string()).parse::<usize>().unwrap_or(cpus),
      enable_tls: env::var("ENABLE_SSL").unwrap_or_else(|_| String::from("true")).parse::<bool>().unwrap_or(true),
      
      allowed_ips_list: env::var("ALLOWED_IPS").unwrap_or_else(|_| String::from("")),
      allowed_ips: vec![],
      
      enable_api_key: env::var("ENABLE_API_KEY").unwrap_or_else(|_| String::from("true")).parse::<bool>().unwrap_or(true),
      api_key: env::var("API_KEY").unwrap_or_else(|_| rand::thread_rng()
        .sample_iter(&Alphanumeric)
        .take(64)
        .map(char::from)
        .collect()),

      fwcloud_script_paths_list: env::var("FWCLOUD_SCRIPT_PATHS").unwrap_or_else(|_| String::from("/etc/fwcloud/fwcloud.sh,/config/scripts/post-config.d/fwcloud.sh")),
      fwcloud_script_paths: vec![],
      
      openvpn_status_files_list: env::var("OPENVPN_STATUS_FILES").unwrap_or_else(|_| String::from("/etc/openvpn/openvpn-status.log")),
      openvpn_status_files: vec![],
      openvpn_status_sampling_interval: env::var("OPENVPN_STATUS_SAMPLING_INTERVAL").unwrap_or_else(|_| String::from("30")).parse::<u64>().unwrap_or(30),
      openvpn_status_request_max_lines: env::var("OPENVPN_STATUS_REQUEST_MAX_LINES").unwrap_or_else(|_| String::from("1000")).parse::<usize>().unwrap_or(1000),
      openvpn_status_cache_max_size: env::var("OPENVPN_STATUS_CACHE_MAX_SIZE").unwrap_or_else(|_| String::from("10_485_760")).parse::<usize>().unwrap_or(10_485_760),
  ...
  pub fn bind_to(&mut self) -> TcpListener {
    ...
    let listener = TcpListener::bind(addr).unwrap_or_else(|_| panic!("Error binding to {}:{}", self.bind_ip, self.bind_port));
